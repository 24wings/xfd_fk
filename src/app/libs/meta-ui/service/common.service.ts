...
import { MetaObjectDataRq } from "../util/meta/MetaObjectDataRq";
import { QueryObject } from "../util/stq/QueryObject";
import { QueryCondition } from "../util/stq/QueryCondition";
import { PageParam } from "../util/stq/PageParam";
  ...
  */
  ...
  async entityQueryPage(metaObject: MetaCom, pageParameter: PageParam): Promise<{ rows: any, count: number }> {
    ...
    if (metaObject.data.presetConditions) {
      if (typeof metaObject.data.presetConditions == 'function') {
        conditions.push(...metaObject.data.presetConditions());
      } else {
        conditions.push(...metaObject.data.presetConditions);
      }
    }

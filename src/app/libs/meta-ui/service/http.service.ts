...
export class MyHttpService {
  ...
  Get(url: string, options?: RequestOptionsArgs, authHeader: boolean = true) {
    ...
    if (!options) options = { headers: new Headers({ Authorization: `Bearer ${localStorage.getItem('token')}` }) }
    else options.headers = new Headers({ Authorization: `Bearer ${localStorage.getItem('token')}` });
    return this.http
  ...
  constructor(public http: Http,
    private _message: NzMessageService, public appConfig: AppConfig,
  ) { }
}
